...
for idx = 1:nr_chunks
    %tic; fprintf('Calculating W (chunk=%d/%d)... ', idx, nr_chunks);
    %----------------------------------------------------------------------
...
%tic; fprintf('Calculating the QR decomposition of Y... ');
[Q,R] = qr(Y,0); % Q:Nk x L+s, R:L+s x L+s
%fprintf('done! (%6.4f/%6.4f sec)\n', toc, toc(start_time));

%% Calculate B (L+s x N)
for idx = 1:nr_chunks
    %tic; fprintf('Calculating B (chunk=%d/%d)... ', idx, nr_chunks);
    %----------------------------------------------------------------------
    % Calculate the range of columns
    %----------------------------------------------------------------------
    if idx < nr_chunks
...
%tic; fprintf('Calculating the SVD of B... ');
[U_hat,S,V] = svds(B,L+s); % U_hat:L+s x L+s, S:L+s x L+s, V:N x L+s
%fprintf('done! (%6.4f/%6.4f sec)\n', toc, toc(start_time));

%% Calculate U
%tic; fprintf('Calculating U... ');
