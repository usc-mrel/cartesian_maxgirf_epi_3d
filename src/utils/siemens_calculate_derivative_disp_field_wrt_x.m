function dDeltak_dx = siemens_calculate_derivative_disp_field_wrt_x(r, theta, phi, R0, alpha, beta, Gref)
% Calculate the Cartesian derivative of a displacement field with respect to x

N = length(r); % number of voxels
dDeltak_dx = zeros(N, 1, 'single');
...
for n = 0:nmax
    ...
    for m = 0:n
        ...
        %------------------------------------------------------------------
        % Calculate d T_{n,m} / dx
        %------------------------------------------------------------------
        dTnm_dx = (r_n_minus_1 / 2) .* (-(1 + delta) * p1 .* cos((m + 1) * phi - mpsi(n+1,m+1)) + ...
                                         (1 - delta) * (n + m) * (n + m - 1) * p2 .* cos((m - 1) * phi - mpsi(n+1,m+1)));

        %------------------------------------------------------------------
        % Calculate the summation
        %------------------------------------------------------------------
        dDeltak_dx = dDeltak_dx + (f * sqrt(alpha(n+1,m+1)^2 + beta(n+1,m+1)^2)) * N_nm * (-1)^m * dTnm_dx;
    end
end

dDeltak_dx = dDeltak_dx / Gref;
