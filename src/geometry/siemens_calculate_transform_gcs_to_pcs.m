function [R_gcs2pcs,phase_sign,read_sign,main_orientation] = siemens_calculate_transform_gcs_to_pcs(dNormalSag, dNormalCor, dNormalTra, dRotAngle)
...
switch main_orientation
    ...
    case CORONAL % == 1
            ...
            elseif ((dRotAngle > 3 * pi / 4) || fGSLAlmEqual(dRotAngle, 3 * pi / 4)) % 135 <= angle <= 180 % ?????
                fprintf('Case 3:\n');
                %----------------------------------------------------------
                % According to TSE, a DICOM image at this case is shifted
                % relative to other DICOM images. So the signs determined with
                % DICOM images may not be correct. So these values should be
                % recalculated according to concomitant field correction using
                % EPI raw datasets.
                % TSE
                read_sign  = -1;
                phase_sign = -1;
                %----------------------------------------------------------
                % correction is OK, but shifted in image space
                %read_sign  = -1;
                %phase_sign = +1;
